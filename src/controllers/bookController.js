...
const getBooksInYear = async function(req, res) {
    let BookInYear = await BookModel.find({ $or: [{ "year": 2021 }] })
    res.send({ msg: BookInYear })
}
const getXINRBooks = async function(req, res) {
    let getXINR = await BookModel.find({
        "prices.indianPrices ": {
            $nin: ["100 ", "200 ", "500 "]
        }

    })
    res.send({ msg: getXINR })
}
const getRandomBooks = async function(req, res) {
    let getRandom = await BookModel.find({ totalPages: { $gt: 500 } })
    res.send({ msg: getRandom })
}
const getParticularBooks = async function(req, res) {
    let particular = req.body
    let getParticular = await BookModel.find(particular)
